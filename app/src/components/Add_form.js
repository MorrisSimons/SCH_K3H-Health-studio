...
function Add_form() {
	const [formFields, setFormFields] = useState([{ name: "", dataType: "" }])
	...
	return (
		<div className="App" class="add_form_container">
			<div class="nameInput">
				<label for="name">Skapa ett formulär</label>
				<br />
				<input name="name" placeholder="Namn på formulär" onChange={setName} />
			</div>
				...
				{formFields.map((form, index) => {
					return (
						<div key={index} class="add_form_field">
							...
							<button onClick={() => removeFields(index)} class="delete">
								Ta bort fält
							</button>
						</div>
					)
