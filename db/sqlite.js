...
async function getTable (tableName) {
  return new Promise((acc, rej) => {
    ...
    try {
      reqText = 'SELECT * FROM ' + tableName
      const result = db.all(reqText, (err, rows) => {
        if (err) return rej(err)
        acc(rows)
      })
    } catch (err) {
...
async function getColumns (table) {
  return new Promise((acc, rej) => {
    ...
    } catch (err) { // If there is an error executing the query, reject the Promise with the error object.
      rej(err)
    }
  })
}
...
async function addTable (table) {
  return new Promise((acc, rej) => {
    ...
    try {
      reqText = 'CREATE TABLE IF NOT EXISTS ' + table.name + ' ('
      // Loop through the tabels and get the data from each one
      for (i = 0; i < table.fields.length; i++) {
        reqText += table.fields[i] + ' ' + table.types[i]
...
async function addIntoTable (table, data) {
  return new Promise((acc, rej) => {
    ...
    try {
      ...
      console.log(data)
      console.log(table)
      reqText = 'INSERT INTO ' + table.name + ' ('
      for (i = 0; i < table.fields.length; i++) {
        reqText += table.fields[i]
        if (i !== table.fields.length - 1) {
          reqText += ', '
        }
      }
...
async function getTeamMembers (teamName, teamWhere) {
  return new Promise((acc, rej) => {
    // Use the db.all() function to execute a SQL query that selects all rows from the table.
    try {
      reqText = 'SELECT * FROM user INNER JOIN team ON user.email = team.email WHERE team.name = "' + teamName + '" AND user.firstName LIKE "' + teamWhere + '%"'
      console.log(reqText)
      db.all(reqText, (err, rows) => {
        // If there is an error executing the query, reject the Promise with the error object.
        if (err) return rej(err)
...
async function getUserType (email) {
  return new Promise((acc, rej) => {
    try {
      reqText = 'SELECT accountType FROM user WHERE email = "' + email + '"'
      console.log(reqText)
      db.all(reqText, (err, rows) => {
        // If there is an error executing the query, reject the Promise with the error object.
        if (err) return rej(err)
...
async function getTeam (email) {
  return new Promise((acc, rej) => {
    try {
      reqText = 'SELECT * FROM team WHERE email = "' + email + '"'
      db.all(reqText, (err, rows) => {
        // If there is an error executing the query, reject the Promise with the error object.
        if (err) return rej(err)
        // If the query is executed successfully, resolve the Promise with an array of table objects.
...
async function getDataWhere (table) {
  return new Promise((acc, rej) => {
    try {
      ...
      // Make an inner join between those tabels and team
      reqText += ' INNER JOIN team ON team.email = ' + tableNames[0] + '.email'
      reqText += ' WHERE ' + whereStatment
      console.log(reqText)
      const result = db.all(reqText, (err, rows) => {
        if (err) return rej(err)
        acc(rows)
    ...
    db.all("SELECT name FROM sqlite_master WHERE type='table'", (err, rows) => {
      ...
      try {
        ...
        for (i = 0; i < rows.length; i++) {
          ...
          reqText = 'DELETE FROM ' + tableName + ' WHERE email = "' + email + '"'
          db.exec(reqText, (err, result) => {
            if (err) return rej(err)
          })
        }
